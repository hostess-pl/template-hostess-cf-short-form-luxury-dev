# Hostess portfolio template — Luxury (Cloudflare Workers)

Dark champagne editorial portfolio for the `hostess-pl` factory. Provisions from the same `hostess.json` contract as `template-hostess-cf`, with a luxury visual system (Bodoni Moda + Outfit, gold-on-charcoal). Tally style **Luksusowy** routes here via `resolveTemplateChoice` in `scripts/tally-normalize-core.mjs`.
...
| `WORKERS_DEV_SUBDOMAIN` | Cloudflare account workers.dev subdomain (e.g. `andreu-oulqaid`) |
...
```json
{"previewUrl":"https://preview-dqvqe25.andreu-oulqaid.workers.dev","clientCode":"…","workerName":"preview-dqvqe25","submissionId":"DqvqE25","analyticsEnv":"preview-dqvqe25","workersDevSubdomain":"andreu-oulqaid"}
```
